...
auto main(main_ctx& ctx) -> int {
    ...
    memory::buffer nonce;
    nonce.resize(32);
    ctx.fill_with_random_bytes(cover(nonce));
    ...
    if(ctx.encrypt_shared(
        ...
        if(ctx.decrypt_shared(view(nonce), view(encrypted), decrypted)) {
    ...
    if(ctx.encrypt_shared_password(view(nonce), "test.password", encrypted)) {
        if(ctx.matches_encrypted_shared_password(
             view(nonce), "test.password", encrypted)) {
