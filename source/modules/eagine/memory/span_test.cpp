...
import <cstring>;
//------------------------------------------------------------------------------
void span_default_construct(auto& s) {
    eagitest::case_ test{s, 1, "default construct"};
    using namespace eagine;
    memory::span<double> a;
...
void span_construct_2(auto& s) {
    ...
    memory::span<const char> acc(cstr, span_size(std::strlen(cstr)));
...
void span_construct_3(auto& s) {
    ...
    memory::span<const char> acc(cstr);
...
void span_construct_4(auto& s) {
    ...
    const memory::span<int> cai(ia, 20);
...
auto main(int argc, const char** argv) -> int {
    eagitest::suite test{argc, argv, "span", 5};
    test.once(span_default_construct);
    test.once(span_construct_1);
    test.once(span_construct_2);
    test.once(span_construct_3);
    test.once(span_construct_4);
