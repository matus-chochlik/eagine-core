...
public:
    ...
    auto arg_value_as(const string_view name, std::type_identity<string_view>)
    ...
    /// @see has_arg
    template <typename T>
    auto arg_value_as(const string_view name) const noexcept
      -> optionally_valid<T> {
        return arg_value_as(name, std::type_identity<T>{});
    }
...
public:
    ...
    /// @brief Returns a hash value for this URL string.
    auto hash_id() const noexcept -> identifier_t;
