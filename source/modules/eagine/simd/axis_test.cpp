...
//------------------------------------------------------------------------------
void vect_axis_int_1(auto& s) {
    eagitest::case_ test{s, 1, "int 1"};
    vect_axis_T_1<int>(test);
}
//------------------------------------------------------------------------------
void vect_axis_float_1(auto& s) {
    eagitest::case_ test{s, 2, "float 1"};
    vect_axis_T_1<float>(test);
}
//------------------------------------------------------------------------------
void vect_axis_double_1(auto& s) {
    eagitest::case_ test{s, 3, "double 1"};
    vect_axis_T_1<double>(test);
}
//------------------------------------------------------------------------------
auto main(int argc, const char** argv) -> int {
    eagitest::suite test{argc, argv, "vect_axis", 3};
    test.once(vect_axis_int_1);
    test.once(vect_axis_float_1);
    test.once(vect_axis_double_1);
