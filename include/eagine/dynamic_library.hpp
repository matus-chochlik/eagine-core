...
public:
    ...
    executable_module(const string_view filename, const int flags)
    ...
    executable_module(const nothing_t, const int flags)
...
public:
    ...
    /// @see open_self
    /// @see is_open
    auto open(const string_view filename) -> auto& {
        _module = _do_open(filename);
        return *this;
    }
